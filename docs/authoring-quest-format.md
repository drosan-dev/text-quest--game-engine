...
- `textPools` - опциональный объект `poolId -> string[]` с глобальными текстовыми пулами (используются через `{{pool:poolId}}`).
- `scenes` - обязательный объект `sceneId -> scene definition`.

Также каждая сцена может объявлять локальные пулы `scene.textPools`. Локальные пулы имеют приоритет над глобальными при использовании `{{pool:poolId}}` внутри этой сцены.
