...
- `textPools` - опциональный объект `string -> string[]` с текстовыми пулами.
...
На верхнем уровне квеста можно объявить `textPools` - набор пулов строк. В тексте узла или выбора можно сослаться на пул с помощью шаблона `{{pool:poolId}}`.
