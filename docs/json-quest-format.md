...
- `textPools` - опциональный объект `string -> string[]` с глобальными текстовыми пулами.
...
- `textPools` опционален и задает локальные пулы для узла (сцены).
...
### Рандомизируемые Текстовые Пулы

Пулы можно объявлять:

- глобально на верхнем уровне `textPools`
- локально на уровне узла через `node.textPools`

В тексте узла или выбора можно сослаться на пул с помощью шаблона `{{pool:poolId}}`. Если в текущем узле объявлен локальный пул с таким id, он имеет приоритет над глобальным пулом.
  ...
  "nodes": [
    {
      ...
      "choices": [
        { "id": "go", "text": "Дальше", "nextNodeId": "end" }
      ]
    }
  ]
}
...
- `poolId` должен существовать в `textPools`.
- пул должен содержать минимум одну строку.
- выбор варианта производится в runtime, а выбранный индекс стабилизируется в `GameState`, чтобы после save/load показанный вариант не менялся.
